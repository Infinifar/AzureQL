...
- [ ] **P2：缩短任务管理与环境变量搜索框。** 当前两处搜索输入框横向占用略长；在不降低点击区域、
  清除按钮可用性和中英文提示可读性的前提下适度收窄，并保持两处宽度、圆角和间距一致。
- [ ] **P2：优化任务管理长按编辑页布局。** 当前长按任务进入的编辑页面内容区域过宽；需要约束表单
  最大宽度及字段/操作区间距，兼顾手机窄屏、横屏和较大屏幕，避免输入控件贴边或视觉重心松散。
- [ ] **P2：脚本文件/文件夹菜单增加“复制路径”。** 在脚本管理右侧三点菜单中，为文件与文件夹均提供
  “复制路径”，复制其完整青龙脚本路径并给出成功提示；不得与现有打开、重命名、删除操作混淆。
- [ ] **P1：脚本管理支持创建文件夹。** 在当前目录提供新建文件夹入口，校验空名称、路径分隔符、同名冲突、
  根目录与嵌套目录边界；创建成功后刷新并定位新目录，失败时保留明确的服务端错误信息。
- [ ] **手动实机验收。** 用户将验证搜索框尺寸、任务编辑页布局、文件/文件夹复制路径及根目录/嵌套目录
  创建文件夹；本轮不代替用户执行上述实机交互测试。
