...
- [x] WebDAV/S3 网络备份支持远端枚举、选择、受限流式下载、格式校验、二次确认和青龙恢复闭环；
  账户级网络存储设置使用规范化、版本化作用域，并从同一账户旧作用域定向迁移，覆盖安装后不再看似清空。
- [x] 客户端拥有的中文文案、业务错误、Toast/Snackbar、空状态、日期、数字和英文单复数已完成 English
  迁移；服务端日志、脚本源码和正文保持原文。用户已于 2026-09-10 确认本轮非 FCM 功能实机测试通过。
- [x] 已加入默认关闭的 FCM Android 接收基础及独立中继安全设计；缺少完整 Firebase 客户端参数时不初始化，
  服务账户私钥不进入 APK。该基础已通过单元测试与构建，但尚未完成中继部署、设备配对、Token 注册、青龙
  Webhook 安装/回滚和端到端实机通知，因此 2.3.4 仅作为技术预览发布，不宣称手机通知同步已经可用。
- [x] 2.3.4 本地门禁通过：网络存储与备份、`core:ui` 本地化、settings/task/env/script/mcp 模块测试、
  `:app:testDebugUnitTest` 和 `:app:assembleDebug` 均成功；正式签名 Release 仍以 GitHub Actions 发布流水线为准。

## 2026-09-09：通知设置与按日系统日志实施

以下两项作为 2.3.3 新功能实施并已完成自动测试与手动真机验收。设置页可见入口由“任务日志”
替换为“系统日志”，但任务详情实时日志、`LogRepository` 的任务日志文件能力以及 MCP 日志工具继续保留。
通知设置只管理青龙服务器当前唯一通知渠道，不等同于把服务端消息同步成 Android 本地通知，也不修改
`config.sh` 中脚本通知变量。字段参考青龙官方
[`systemNotify`/`notificationInfo` 文档](https://qinglong.online/guide/user-guide/built-in-api)；其中
`systemNotify` 是脚本内置 API，本客户端保存配置仍使用面板 REST API `GET/PUT /api/user/notification`。

### A. P1：通知设置子页

- [x] **接通通知配置数据契约（代码与单测完成）。** 在 `AutoPanelApiService` 增加 `GET/PUT /api/user/notification`，以
  `JsonObject` 读取和提交动态字段，并新增独立 `NotificationRepository`；禁止把访问令牌、机器人密钥、
  SMTP 密码等写入日志、DataStore、`SavedStateHandle` 或备份。读取失败、HTTP/API 失败与协程取消必须
  沿用现有 Repository 错误和取消语义。
- [x] **建立版本兼容的通知字段注册表（代码与单测完成）。** 覆盖官方文档中的 Gotify、Go-CQHttp、Server 酱、PushDeer、
  Bark、群晖 Chat、Telegram、钉钉、企业微信机器人/应用、Aibotk、iGot、PushPlus、WePlusBot、Email、
  PushMe、飞书、Webhook、Chronocat、Ntfy 与 WxPusher，并兼容当前青龙上游界面的 `wxPusherSpt`、
  `openiLink` 及 `feishu`/`lark` 命名差异。字段类型、必填项、默认值、枚举与帮助文案由注册表统一提供；
  未识别类型只读展示并禁止覆盖，避免新版服务端配置被旧客户端静默丢失。
- [x] **实现设置 → 通知设置独立子页（代码及实机验收完成）。** 页面按“通知方式 → 当前方式字段”的结构动态显示；密钥字段默认
  遮罩并提供临时显隐按钮，切换方式时只提交新方式允许的字段，同一方式编辑时保留客户端尚不认识的原字段。
  “关闭”提交空 `type`；保存按钮明确标为“保存并发送测试通知”，提交期间防重复点击，成功、测试发送失败
  与网络失败分别反馈。通知配置仅停留在当前 ViewModel 内存，进程重建后重新从服务器读取。
- [x] **通知自动测试与实机验收。** Repository 测试覆盖动态 JSON、空类型、未知字段、服务端失败和取消；
  ViewModel/字段注册表测试覆盖必填校验、模式切换、敏感字段遮罩、未知类型只读、防重复提交及一次性消息；
  Compose 测试覆盖动态字段和中英文语义。真机使用真实通知渠道确认保存时收到青龙测试通知，并检查
  进程重建与账户切换不泄露或串用配置；其余通知渠道作为后续兼容矩阵，不阻断本轮完成定义。
  - [x] 2026-09-09 自动测试源码完成：动态字段、同渠道未知字段保留、跨渠道字段隔离、必填拦截、
    服务端失败与协程取消均有 JVM 用例；通知 Compose 页面测试源码编译通过。密钥只存在于 ViewModel 内存，
    UI 默认使用密码遮罩，未接入 DataStore、`SavedStateHandle`、备份或日志。
  - [x] 2026-09-09 Android 16 覆盖安装后，设置入口、现有通知渠道读取、动态表单和测试保存按钮均正常显示；
    本轮未读取、修改或提交真实渠道密钥。
  - [x] 2026-09-09 首次真实发送复验暴露两项问题并完成代码修复：按钮加载指示器原先只约束高度，旋转时
    横向拉伸、中心偏移并越出按钮；现固定为居中的 20 dp 正方形并补充 Compose 尺寸回归。测试通知失败时，
    青龙旧版 `testNotify()` 会让渠道异常直接成为 HTTP 500；客户端现安全分类 DNS、拒绝连接、超时、认证和
    404 等常见原因，其他情况明确提示服务端测试失败且设置未保存，不展示可能包含密钥的原始异常正文。
  - [x] 2026-09-09 修复构建覆盖安装后再次发送：客户端请求与青龙通知配置契约一致，服务端仍返回不带渠道
    详情的 HTTP 500，确认不是客户端崩溃且配置未保存。随后确认通知服务域名要求 mTLS：App 连接青龙时
    使用的客户端证书不会传递给青龙后端发起的通知渠道请求，因此改用青龙容器可直连的内网 IP 继续验证。
    无安全详情时的提示现同时引导检查渠道配置、青龙容器网络和“系统日志”；实际渠道送达仍保持未通过，
    不能误标为客户端已完成发送。
  - [x] 2026-09-09 用户手动实机验收：真实通知渠道“发送测试通知并保存”成功；进程重建及账户切换后，
    通知配置均从当前服务器重新读取，未出现跨账户串用。

### B. P1：设置入口改为按日系统日志

- [x] **接入青龙系统日志流（代码与单测完成）。** 新增流式 `GET /api/system/log`，同一天同时作为 `startTime`/`endTime`，
  `limit` 固定不超过 1 MiB；读取原始响应正文及 `X-QL-Log-Total`、`X-QL-Log-Truncated` 响应头，在客户端
  再执行字节上限保护。空正文是有效的“当日无日志”，不得按 JSON 解码或误报接口失败。
- [x] **按服务器日期逐日分类（代码与单测完成）。** 从 `GET /api/system/config` 的 `timezone` 生成最近 7 个服务器自然日，
  时区缺失或非法时回退设备时区；日期倒序排列、按点击懒加载，支持下拉刷新当前日期。打开某日后复用
  `WindowedLogViewer` 展示，清楚标记空日志与 1 MiB 截断。首版不提供 `DELETE /api/system/log` 的全量清空，
  避免把按日浏览误做成不可恢复的全局删除。
- [x] **替换可见入口但保留任务日志能力（代码完成）。** 设置“服务器管理”中的“任务日志”改为“系统日志”，导航到新的
  `SystemLogRoute/SystemLogScreen`；不删除任务页运行日志、`GET /api/logs`/`detail`、任务日志 Repository 方法
  或 MCP 日志工具。旧任务日志文件浏览页不再从设置进入，防止两个不同日志概念继续共用同一标题。
- [x] **系统日志自动测试与实机验收。** 数据层测试覆盖日期查询、原始正文、空响应、截断响应头、超限保护、
  服务端失败和取消；ViewModel/Compose 测试覆盖服务器时区、最近 7 天排序、按需加载、刷新、空态、截断提示
  与返回/下拉关闭。真机使用至少今天和前一天的日志验证分类、正文、滚动到底部和退出动画，并确认任务详情
  实时日志及 MCP 日志读取没有回归。
  - [x] 2026-09-09 自动测试源码完成：日期查询、空正文、截断响应头、客户端超限保护、取消传播、
    最近 7 天倒序和按点击懒加载均有 JVM 用例；系统日志 Compose 页面测试源码编译通过。
  - [x] 2026-09-09 Android 16 首轮实机发现测试实例对带 `limit` 的系统日志请求返回 HTTP 400；确认
    青龙 2.20.x 路由尚不接受该参数，而当前上游已支持。客户端增加 400 后自动去掉 `limit` 的兼容重试，
    旧服务端响应仍由流式读取在本地强制限制为 1 MiB，并补充回归测试。
  - [x] 2026-09-09 Android 16 兼容构建覆盖安装后，系统日志显示 7 个倒序日期；当天与前一天正文均成功加载，
    未再出现 HTTP 400；日志面板下拉关闭后正常返回日期列表，应用进程保持运行。
  - [x] 2026-09-09 用户手动实机验收：较长系统日志可正常滚动到底部，下拉刷新正常；任务详情实时日志
    与 MCP 日志读取快速回归均通过，未发现功能或交互回归。

### 实施顺序与完成门槛

1. 先完成通知与系统日志的 API/Repository 契约及红绿单测。
2. 再实现通知动态表单、系统日志按日状态机与独立导航页面。
3. 最后补齐 Compose 测试源码编译、全量 JVM 回归、Lint、Debug APK 和上述真机清单。

完成门槛：相关模块测试、全量 `testDebugUnitTest`、`lintDebug`、通知/系统日志 Compose 测试源码编译和
`:app:assembleDebug` 全部通过；凭据不落盘、不进入日志，不支持的服务器字段不被覆盖，任务详情/MCP 日志能力
保持可用。2026-09-09 规划基线 `e513a945` 上，`:core:data:testDebugUnitTest`、`:feature:log:testDebugUnitTest`
和 `:feature:settings:testDebugUnitTest` 已通过。实施后相关三模块 JVM 测试、两组 Compose 测试源码编译、
全项目 `testDebugUnitTest`、`lintDebug` 与 `:app:assembleDebug` 均已通过；Debug APK 已生成。
2026-09-09 用户已完成实际通知发送、进程重建与账户隔离、长系统日志滚动/刷新，以及任务详情实时日志和
MCP 日志读取的手动真机验收；本节完成门槛现已全部满足。

## 2026-09-06：2.3.1 发布后 UI 与脚本管理需求

以下项目先进入待办，不计入 2.3.1 已完成定义；实现后的真机交互验收均由用户手动执行。

- [x] **P2：缩短任务管理与环境变量搜索框（代码完成，待手动实测）。** 两处搜索输入框统一缩短为可用标题
  区域的 90%，并把提交搜索按钮向左预留屏幕宽度的 10%；保留返回、自动过滤、点击区域及中英文提示。
- [x] **P2：优化任务管理长按编辑页布局（代码完成，待手动实测）。** 编辑弹窗改为与环境变量编辑相同的
  Material 3 标准宽度，不再强制 520 dp/90%；实例模式改为等宽单行选项并以省略保护大字体和英文布局。
- [x] **P2：脚本文件/文件夹菜单增加“复制路径”（代码完成，待手动实测）。** 文件与文件夹右侧三点菜单均新增
  “复制路径”，复用规范化青龙脚本路径和复制成功提示；文件原有长按复制仍保留。
- [x] **P1：脚本管理支持创建文件夹（代码完成，待手动实测）。** 根目录工具栏及文件/文件夹三点菜单均提供
  “新建文件夹”；接入青龙官方 `POST /api/scripts` 的 `directory` 请求字段，支持当前目录与嵌套目录，并在
  提交前拦截空名称、`.`/`..`、路径分隔符、控制字符、超过 255 字符及当前脚本树中的同名冲突。创建期间禁止
  重复提交，成功后刷新脚本树，服务端失败详情保留在弹窗及 Snackbar。
- [x] **P2：隐藏订阅链接后的订阅列表布局优化（代码完成，待手动实测）。** 订阅卡片不再显示可能包含仓库
  信息或凭据参数的 URL，也不保留空白占位；状态与定时规则并入标题内容区，统一紧凑内边距并限制定时规则
  为单行省略，保留点击卡片查看日志及运行、停用、编辑、删除操作。
- [x] **自动验证。** 全项目 44 个测试套件、256 项 Debug 单元测试全部通过（0 失败、0 错误、0 跳过）；
  任务/环境变量模块编译和脚本 Compose Android 测试源码编译通过；`:app:assembleDebug`、`:app:lintDebug`
  全部通过。
- [ ] **手动实机验收。** 用户将验证搜索框尺寸、任务编辑页布局、文件/文件夹复制路径及根目录/嵌套目录
  创建文件夹，并确认隐藏链接后的订阅卡片布局；本轮不代替用户执行上述实机交互测试。

## 2026-09-07：2.3.2 导航、紧凑布局与实时日志修复

- [x] **P1：六段连续左右滑动导航。** 主页面改为 Compose `HorizontalPager`，与底部导航共享页面状态；
  脚本模块保留内部“脚本/订阅”Pager，并依靠 Compose 嵌套滚动在边界把手势交给主 Pager，形成
  “首页 ↔ 任务 ↔ 脚本（脚本）↔ 脚本（订阅）↔ 环境 ↔ 设置”的连续顺序。从任务打开脚本仍会携带路径
  并切换到脚本页。XT2551-3（Android 16）已通过 ADB 正向、反向各五次连续滑动及底栏跳转冒烟。
- [x] **P2：环境变量卡片与订阅卡片统一紧凑尺寸。** 备注移入名称内容列，不再叠加在整条操作行下方；
  卡片采用与订阅一致的水平 12 dp、垂直 10 dp 内边距。启停仍使用原生 `Switch`，最终视觉缩放为 78%，
  置顶操作和可访问性描述保持不变。`EnvItemTest` 4/4 已在同一 Android 16 设备通过。
- [x] **P1：任务实时日志终态重复显示。** 青龙在任务结束时可能忽略请求游标并返回不带 `offset`/
  `nextOffset` 的完整日志；旧逻辑用请求 offset 伪造增量游标，导致整份终态日志重复追加。现仅在服务端明确
  返回游标时按增量处理，否则将内容视为从 0 开始的完整快照并替换现有窗口；保留结束后的最后一次补拉，
  避免遗漏末尾输出。仓库映射与 ViewModel 终态快照回归测试通过，用户实机确认恢复正常。
- [x] **2.3.2 本地发布验收。** Debug APK 已多次在 XT2551-3 覆盖安装并保留账户数据；本地发布验证通过
  44 个测试套件、258 项单元测试（0 失败、0 错误、0 跳过），Android Lint、备份/环境变量/脚本 Compose
  测试源码编译及 Release APK 构建全部成功。GitHub Actions 与签名 Release 结果以发布页为准。

## 2026-09-06：2.3.0 发布内容与 mTLS 续连

- [x] **P2：备份导出内容默认折叠。** “选择导出内容”初始只显示“基础设置”“配置文件”和“脚本文件”，其余模块
  保持原有勾选状态但不占用页面高度；提供“展开全部/收起”操作，展开后显示脚本、日志、依赖及各类缓存等全部模块。
  折叠只属于当前页面的临时 UI 状态，不改变导出模块选择，也不写入持久化配置。2026-09-06 已完成代码实现；
  按用户要求不执行本轮实机测试。
- [x] **P2：网络导出显式选择目标。** 点击“导出到网络存储”后显示 WebDAV 与 S3 目标选择；仅列出已经
  “保存并测试连接”成功的目标，未配置任何目标时保持禁用并引导进入“网络存储设置”。选择目标后仍沿用同一
  WorkManager 导出/上传、取消、进度、通知和失败分类链路，不允许因切换目标携带另一目标的凭据。该选择能力
  已在此前的网络存储第一阶段实现，并由用户于 2026-09-06 确认，无需重复开发。
- [x] **P1：mTLS 临时会话密钥过期后自动复用证书续连（代码完成，待跨日实测）。** 新反馈场景为：账户保持登录约一天后，服务端临时
  “会话密钥”失效，App 后续请求未自动重新执行证书交换，界面反而显示域名解析类错误；当前只能通过切换账户并
  重新登录恢复。2026-09-06 已将启用客户端证书的缓存 TLS 客户端最长生命周期限制为 12 小时；后续任一 API 或
  WebSocket 建连都会在进入网络请求前，以当前账户已保存在安全存储中的 PKCS#12、证书密码与私有 CA 重建独立
  SSLContext，并重新执行完整证书握手，不要求切换账户。轮换在客户端注册表的同一临界区内完成，并发请求只会生成
  一个新客户端；未启用 mTLS 的普通连接继续长期复用，避免无谓损耗。本修复不静默重放账号密码，也不把真正的
  Token 过期、DNS 故障或证书失效伪装成 mTLS 续连成功。
  - [ ] 跨日实机验收由用户执行：保持同一 mTLS 账户登录至少一天，等待会话密钥自然失效后直接刷新首页及进入
    任务/脚本页面，确认无需切换账户即可自动恢复；再覆盖并发请求、真正断网、错误证书和服务端不可达。按用户要求，
    本轮不执行自动或即时实机测试，待跨日结果回填后再关闭本项。

## 2026-09-05：2.2.8 发布后问题与后续需求

本节记录 2.2.8 发布后反馈，均不因发布前测试通过而自动关闭。修复时需要重新建立可复现步骤、模块测试和
对应真机验收证据。

- [x] **P1：设置 → 任务日志滑动至底部仍会持续抖动。** 2.2.8 实机反馈确认问题仍存在，重新打开此前的
  “触底抖动”缺陷。需要分别记录静态日志、运行中增量日志、日志轮转和 256 KiB 窗口裁剪四种状态，核对
  `LazyListState`、触底自动跟随、窗口头部裁剪与每 2 秒追加是否互相回写滚动位置。验收需包含触底静置、
  手动离开底部、任务终态以及退出重进，并同时保存屏幕录像、帧时间和列表位置变化证据。
  - [x] 2026-09-05 代码修复：移除日志正文 `LazyColumn` 与 `ModalBottomSheet` 的嵌套滚动竞争，改用与
    “任务管理 → 日志”相同的独立覆盖层；只有顶部拖动柄处理下拉关闭，正文触底余量不再拖动整个面板。
  - [x] 2026-09-05 XT2551-3（Android 16）复验：打开 20 MiB 日志并连续滑至底部，静置采样 8 帧时正文区域
    完全一致（唯一像素变化位于系统状态栏）；返回键和顶部拖动柄均能关闭覆盖层，退出后的系统日志无
    `showSoftInput`/IME 显示请求。反向滚动不会带动覆盖层，未再出现触底抖动。
- [x] **P1：脚本管理 → 订阅日志没有形成可见的流式更新。** 当前入口能够读取日志，但用户在订阅运行期间
  看不到持续追加的实时日志。需要核对青龙订阅日志接口的 `offset`/`limit`/`tail` 返回语义，补齐初次尾部读取、
  增量游标追加、轮转/截断重置、终态停止、离开页面取消和最新 256 KiB 上限；不得以固定刷新整份正文冒充流日志。
  单测与实机验收应覆盖运行中确有新行、无新增内容、服务端轮转、网络恢复和结束状态。
  - [x] 2026-09-05 代码与模块测试：初次尾部读取后按游标增量追加，允许旧列表状态下 3 轮启动探测；每轮先拉取
    最终日志再确认订阅终态，避免快速结束时丢失末尾内容；界面明确显示“实时更新中”。新增运行中终态追加、
    旧闲置状态转运行两项回归测试并通过。
  - [x] 2026-09-05 XT2551-3（Android 16）运行 `BilibiliToolPro` 订阅复验：执行期间显示
    “拉取日志 · 实时更新中”，日志正文随拉取阶段增长；约 3 秒进入终态后标记自动消失，最后成功行与执行结束行
    均保留。实测青龙 2.21.0 的该接口正文可用但游标元数据为零值，客户端已增加“全量快照 + UTF-8 虚拟游标”
    兼容分支，并对带游标的重叠响应裁剪已消费前缀；修复后终态正文只出现一次、无重复追加。模块测试新增正常增量、
    重叠全量响应、零游标快照和陈旧闲置状态转运行用例，均通过。
- [x] **P1：MCP 客户端发送消息时连接被重置。** 2.2.8 收到以下 release 堆栈，初始证据显示连接在
  OkHttp 读取 HTTP/1.1 响应头之前被对端或中间网络重置，尚不能据此归因于 MCP SDK、Android 服务或客户端：

  ```text
  io.modelcontextprotocol.kotlin.sdk.types.McpException:
  Error while sending message: Connection reset
  Caused by: java.net.SocketException: Connection reset
  at okhttp3.internal.http1.Http1ExchangeCodec.readResponseHeaders(...)
  ```

  2026-09-05 通过 `adb forward tcp:18765 tcp:18765` 稳定复现：应用进程启动后首个请求返回 401，随后包括
  新建 TCP 连接在内均收到空响应，而前台服务仍存活；仅重启应用进程可再恢复一个请求。改用 Ktor CIO 服务端并
  显式等待监听器就绪，同时纳管传输协程生命周期与端口占用失败状态；移除不再需要的 Netty 依赖、R8 keep 和
  可选原生传输配置。干净安装后的 Android 16 实机连续 10 次返回 401，随后第 11、12 次按限流策略稳定返回 429，
  服务和进程持续存活；模块测试覆盖“连续拒绝后仍可完成官方客户端授权握手”和“端口占用不会遗留运行态”。
- [x] **P2：MCP 服务增加“局域网可访问”与静默允许受控写入/执行。** 2026-09-06 已实现两个相互独立、
  默认关闭的高风险开关：服务监听范围可由 `127.0.0.1` 切换为全部接口，单个 Agent 可在已取得 Phase 2 Scope 后
  选择跳过逐次手机确认。开启任一高风险能力前均显示完整风险说明并要求设备身份验证；关闭写入 Scope 时会自动撤销
  该 Agent 的静默授权。静默模式只让当前已注册的 `CONTROLLED_WRITE` / `EXECUTION` 工具首次请求直接进入持久化
  `RUNNING` 状态，账户绑定、Scope、参数/路径上限、幂等、单 Agent 写入串行化、脚本哈希冲突检查和脱敏审计继续生效，
  不开放任意 Shell、任意 HTTP、凭据读取或未建模的删除操作。

  局域网模式仅接受 Host 为本机实际非蜂窝局域网/VPN 接口地址的请求，浏览器 Origin 还必须与请求 Host 匹配；
  默认界面只显示首选局域网 IPv4，其余 VPN/IPv6 地址折叠展示。设置页明确标注当前仍为明文 HTTP Bearer Token，
  只适合可信局域网，运行时禁止更改监听范围，并保留前台通知的一键停止、Agent 撤销、限流与审计。Android 16 实机
  已确认默认监听仅落在回环地址；开启后套接字切换为全接口，电脑直连手机 `10.181.216.110:18765/mcp` 得到预期
  `401 Unauthorized`，证明局域网链路可达且鉴权未绕过；界面折叠态仅显示该 IPv4 与“其他 2 个地址”，展开后可见
  VPN IPv4/IPv6。测试结束已停止服务并恢复“仅本机”设置。`core:mcp` 36 项、`feature:mcp` 1 项单元测试及
...
- [x] **2026-09-04～09-05 工作与文档对账。** 9 月 4 日的性能/大脚本/日志流/输入法与返回动画修复，及
  ...
  非预发布版本公开发布，Android CI 在提交 `08c6d9165a2f5bd5731ac26397874572139f0414` 成功；发布资产
  `AzureQL-2.2.9-release.apk`（6,910,818 bytes）的 SHA-256 为
  `3a0996b6d46e4e5505a963986343fcc9a7569871d90e7aff94575de4bd82d7de`。发布页：
  <https://github.com/Infinifar/AzureQL/releases/tag/v2.2.9>；流水线：
  <https://github.com/Infinifar/AzureQL/actions/runs/33960806270>。

### 2026-09-03 实机回归新增缺陷（修复与复验中）

- [x] **P0：MCP minified 启动与认证握手已恢复。** 2026-09-03 曾通过扩大 Netty runtime keep 范围解决
  `Class gl3 does not have a public non-arg constructor` 与 `Can't find '[toLeakAwareBuffer]'`；该临时路径已在
  2026-09-05 的 connection-reset 修复中由 Ktor CIO 服务端替代，Netty 依赖和对应 R8 规则已移除。
...
- [x] **P1：任务日志滚到底部持续抖动（2.2.8 反馈后已再次修复）。** 此前问题曾收敛为日志选择层包裹整棵
  `LazyColumn` 后与底部抽屉
...
- [x] “设置 → 账户与服务器”展示账户卡片/列表，显示脱敏服务器地址、别名、认证方式、证书状态、最近
  使用时间和当前账户标记；绝不显示 Token、密码或 Client Secret。
- [x] 已保存账户可在面板内预览、编辑服务器地址/HTTP 安全策略/用户名、重命名别名、删除、置顶/排序和立即切换；
  新增账户仍进入登录页，避免在设置内复制完整登录流程。
- [x] 切换前取消旧账户的 HTTP 请求与 WebSocket，并拦截仍有备份、恢复或网络导出 WorkManager 任务的切换；
  候选账户使用独立连接配置重新认证，只有收到新 Token 后才原子更新 `SessionManager`、
  `ApiClientRegistry`、账户隔离缓存/草稿/MCP Scope，并刷新当前页面；失败时保留原账户可用。
- [x] 删除必须二次确认并清理该账户的加密凭据、响应/脚本缓存、草稿、WebDAV/S3 配置和本地 Agent 绑定；
  当前账户不可在仍有不可恢复的上传/恢复任务时静默删除。
- [x] 三账户实机验收：密码、Client ID、mTLS/错误证书账户可连续切换，并可在设置内编辑别名、删除
  非当前/当前账户，不再要求退出到登录页才能管理已保存账户。2026-09-06 由用户确认实机通过。
- [ ] 扩展验收仍保留进程重建、断网/恢复、缓存隔离和英文/大字体/TalkBack。
...
- [x] 新增独立“账户与服务器”设置子页；账户卡片支持中英文、大字体自适应布局所需的纵向信息结构，
  展示脱敏域名/IP、别名、用户名或 Client ID、认证方式、TLS/mTLS/私有 CA 状态、最近使用时间和当前标记。
- [x] 历史账户继续以 `SessionManager.accountsFlow` 为唯一数据源；`StoredAccount` 以向后兼容默认字段补充
  证书路径、私有 CA 和最近使用时间，密码、Client Secret、证书密码继续只存 Android Keystore 加密区。
- [x] 支持别名/地址/用户名/HTTP 策略编辑、置顶、上移、下移和二次确认删除。当前账户仅修改别名时不重登；
  连接身份变化时先用保存的凭据重新认证，失败或取消 2FA 会回滚历史记录并保持原会话。
- [x] 密码、Client ID 与 mTLS 账户可在设置内直接切换；密码账户遇到 2FA 时在当前子页完成验证码输入。
  切换成功后重建应用内导航根页面，避免保留旧账户 ViewModel 状态；失败不清空旧 Token。
- [x] 新导入客户端证书和私有 CA 改为会话独立随机文件名，不再由多个账户共同覆盖固定文件；删除账户会
  清除对应加密登录/证书凭据，并在没有其他账户引用时删除其 App 私有证书文件。
- [x] 切换前取消缓存 OkHttp Dispatcher 中的在途调用；备份、恢复、WebDAV/S3 网络导出仍在运行时拒绝
  切换或删除最后一个当前账户，避免 WorkManager 跨账户继续执行。
- [x] 模块验证：`:core:data:testDebugUnitTest`、`:core:mcp:testDebugUnitTest`、`:feature:backup:testDebugUnitTest`、
  `:feature:login:testDebugUnitTest`、`:feature:settings:testDebugUnitTest` 与 `:app:compileDebugKotlin` 全部通过；新增覆盖候选连接成功/失败回滚、
  2FA、当前账户标记、非当前账户删除和服务器地址脱敏。
- [x] 删除时按账户作用域立即清理 Room 响应缓存和带作用域元数据的脚本草稿；同时从本地 MCP Agent
  allow-list 移除该账户，若 Agent 已无任何允许账户则一并撤销。旧版本未带账户元数据的草稿无法可靠反查归属，
  继续由既有 8 天/容量淘汰机制清理，避免误删其他账户草稿。
- [x] WebDAV/S3 设置改为以不可逆账户哈希分区；升级后旧版全局配置只迁移给升级时的当前账户，后续账户
  使用独立配置。删除账户只移除该分区的连接信息和加密密钥，不会清空其他账户的网络存储目标。
- [x] 2026-09-10 修复覆盖安装后网络存储设置看似清空：账户作用域改为版本化的 host/trimmed-user/auth
  稳定散列，并仅从同一当前账户的旧 `historyId` 作用域定向迁移；页面在异步读取完成前显示加载状态，
  切换账户时也不会保留上一账户的未保存草稿。`:core:data:testDebugUnitTest` 与
  `:feature:backup:testDebugUnitTest` 已通过。
- [x] 设置子页已支持直接添加、替换或移除 mTLS 客户端证书与私有 CA；文件先原子复制到 App 私有目录，
  证书密码仅写入 Android Keystore 加密区。当前账户保存后使用候选 TLS 配置重新认证，失败或取消 2FA
  会恢复旧账户记录、旧证书密码与原会话；成功后只清理不再被任何账户引用的旧证书文件。
- [x] 自动验证新增覆盖非当前账户证书保存、当前账户错误证书切换失败后的记录/密码回滚与新文件清理；
  `:core:data:testDebugUnitTest`、`:feature:settings:testDebugUnitTest`、`:app:compileDebugKotlin` 和单 worker
  `:app:assembleDebug` 全部通过。新 Debug APK 已于 2026-09-06 覆盖安装至 Android 16 实机并保留应用数据。
- [x] 2026-09-06 用户确认账户子页内直接替换证书实机通过。
- [ ] 证书扩展回归仍覆盖无密码/有密码 PKCS#12、私有 CA、错误密码/损坏文件、取消选择、当前与非当前
  账户替换，以及替换失败后的旧会话持续可用。
- [x] 账户卡片完成第二轮 UI 收敛：取消整卡点击切换，改为带文字的显式“切换”和“编辑”操作；置顶、
  上移、下移、删除集中到“更多”菜单，当前账户使用状态标记与描边区分，长地址/用户名使用省略保护。
  中文窄屏实机截图已复核，菜单无越界；`:feature:settings:testDebugUnitTest` 与单 worker
  `:app:assembleDebug` 通过并覆盖安装至 Android 16 实机。

### 3. 内置代码编辑器与按扩展名语法高亮

- [x] 在脚本详情增加应用内编辑入口；按扩展名大小写不敏感地自动选择模式：初始支持 Python
  (`.py`)、JavaScript/TypeScript (`.js/.mjs/.cjs/.ts`)、Shell (`.sh/.bash`)、JSON (`.json`)、
  YAML (`.yml/.yaml`)；未知扩展名使用纯文本模式，并允许手动切换。高亮只改变显示，不执行代码。
  - [x] 2026-09-05 首期实现选择 Sora Editor `0.24.6`，使用其 Android 原生编辑、行号、块引导线和
    Monarch 增量高亮；常用脚本由扩展名自动选型，标题栏可手动切换模式。为避免完整语言包让未混淆
    Debug APK 膨胀，只保留首期 5 种 Apache-2.0 Monarch 语法定义，并使用这些规则已兼容的 Kotlin Regex，
    不打包当前未使用的 Oniguruma/Re2J 后端。JSON 首期复用 JavaScript
    词法规则（JSON 是其语法子集），解析或未知类型回退 `EmptyLanguage`，不阻断查看、编辑或保存。
    普通 INLINE 脚本的只读查看和编辑均使用同一 Sora 组件；PAGED 大文件继续使用既有 8 KiB 分段预览，
    防止高亮把整份大文件重新送入布局。首期不接入 LSP、代码执行或网络语法服务。
  - [x] 2026-09-05 自动与真机验证：新增扩展名大小写、别名与未知类型降级单测；
    `:feature:script:testDebugUnitTest`、`:app:assembleDebug`、`:app:assembleRelease` 均通过。
    XT2551-3（Android 16）打开实际 `51加白.py`，确认深色主题下行号、注释、关键字、字符串和普通标识符
    均可读；Python/JavaScript/TypeScript/Shell/JSON/YAML/Text 七个菜单项连续切换无崩溃。进入编辑时输入法
    正常显示，取消后恢复原文且 `mImeWindowVis=0`、`mInputShown=false`。最终 minified Release 为
    7,296,042 bytes，较未集成 Sora 的本地 2.2.9 Release（6,896,522 bytes）增加 399,520 bytes（约
    0.38 MiB）。本次未向青龙服务端提交修改，实际保存/冲突/断网仍归入下一条草稿与保存链路验收。
  - [x] 发布前第三方许可证声明：新增 `THIRD_PARTY_NOTICES.md`，明确 Sora Editor `0.24.6` 使用
    LGPL-2.1、Monarch 语法定义使用 Apache-2.0，并提供固定版本、对应源码与许可证入口。项目继续固定
    `0.24.6`，不跟随动态版本；两种许可证均允许在遵守条款时商业使用，并非仅限非商业用途。声明同时列出
    LGPL 组件替换/重建、修改库源码开放、归属保留及版本升级复核要求。升级前仍须重新执行输入法、光标/选区、
    主题、高亮与 minified 构建回归。
...
2026-09-06 已交付第一阶段：设置入口和页面统一改名为“备份与恢复”，原“导出到文件”改为
“导出到本机存储”，其下新增“导出到网络存储”。WebDAV 与 S3 的连接表单已移入独立“网络存储设置”子页，
备份主页只显示配置状态和入口；只有“保存并测试连接”成功的目标才允许网络导出，同时配置两种目标时导出前选择。
WebDAV 密码以及 S3 Access Key/Secret Key 均使用各自独立的 Android Keystore AES-GCM 密钥加密保存。
后台任务先把青龙官方导出流写入 App 私有缓存文件，再通过独立（不携带青龙 Token）的 OkHttp 客户端流式上传，
完成或失败后清理缓存。远程目录会以 `PROPFIND` 检查并逐级 `MKCOL`，归档使用唯一时间戳文件名和
`If-None-Match: *` 防止静默覆盖；进度、取消、自动重试、前台通知和跨页面观察继续沿用现有 WorkManager 链路。
S3 已按 RikkaHub 的轻量客户端思路独立实现 SigV4，不引入完整 AWS SDK；支持自定义端点、存储桶、`auto`/显式区域、
路径样式、远程目录、流式 `PUT`、`If-None-Match: *` 与服务端区域提示后单次重签。实现依据 AWS 官方 SigV4 与
条件写入规范，未复制参考项目源码或凭据持久化方式。

- [ ] 设置中增加按账户隔离的 WebDAV 配置：服务器 URL、远程目录、文件名模板、用户名/应用密码，
  可选自签名证书信任配置；凭据使用 Android Keystore 加密，不进入 Room、日志、URI 或备份归档。
  - [x] 已完成服务器 URL、远程目录、用户名/应用密码、Keystore 加密、连接测试及按青龙账户隔离；旧版全局
    配置只迁移给升级时的当前账户。文件名模板和自签名证书信任仍待后续实现。
- [ ] 导出流程调用现有青龙 `api/system/data/export` 能力并流式转发到 WebDAV `PUT`；不把整份归档
  ...
  - [x] 第一阶段已完成私有临时文件 + 流式 `PUT`，使用唯一秒级时间戳和 `If-None-Match: *`；
    `.part` + MOVE 以及服务端 HEAD/长度/哈希复核仍待实现。
- [ ] 保留现有五阶段进度、取消、WorkManager/前台通知和错误分类；WebDAV 认证失败、TLS 失败、
  目录不存在、空间不足、超时、断网、冲突和服务端导出失败必须分别提示。失败或取消清理 `.part`，
  重试不得重复生成不可识别的半成品。
  - [x] WorkManager、前台通知、导出/上传分阶段进度、取消、私有缓存清理和网络异常重试已接入；
    认证、404、409、412、507、TLS/证书、超时与普通 I/O 已提供脱敏提示；失败 PUT 会尽力删除远端同名半成品，
    真实 WebDAV 服务上传已由用户于 2026-09-06 手动测试通过；其余异常矩阵仍需专项验收。
- [ ] 明确覆盖策略（拒绝、询问、版本化文件名），支持按账户/日期分目录；默认不覆盖已有归档。
  网络恢复已具备“远端选择、下载校验、再次确认后覆盖”的独立闭环；上传成功仍不等于恢复实测通过。
- [ ] 验收：MockWebServer/WebDAV 兼容服务与真实 WebDAV 各覆盖小/大归档、401/403、409、404、507、
  ...
  - [x] JVM 协议测试已覆盖根目录握手、中文多级目录 URL 编码、逐级建目录、Basic Auth、归档流式上传、
    完整长度、进度回调、401 脱敏分类，以及 507 后删除远端半成品；真实 WebDAV 上传已手动通过，小/大归档
    分档、其余异常矩阵与恢复下载实机流程仍未验收，不能据此标记全部完成。
- [x] **网络恢复第二阶段代码与自动测试（2026-09-10）。** “备份与恢复”新增从 WebDAV/S3 选择远端归档；
  WebDAV 通过 `PROPFIND Depth: 1` 读取直属文件，S3 通过签名的 `ListObjectsV2` 分页读取配置前缀，均只展示
  `.tgz/.tar.gz/.gz`，并按修改时间倒序显示文件名、大小与时间。选中后由 WorkManager 流式下载到应用私有
  临时目录，同时校验声明长度和实际字节上限；远端标识必须位于已配置目录，XML 列表有 2 MiB 上限并拒绝
  DTD/外部实体。下载完成后复用既有青龙导入、格式校验、二次覆盖确认、激活恢复、健康检查及重新登录链路；
  取消、失败和完成都会清理临时文件，进度和前台通知可跨页面观察。备份模块 40 项 JVM 测试全绿，覆盖
  WebDAV/S3 列表过滤、签名下载、路径越界拒绝、未知长度实际流量上限、恶意 XML 拒绝及 ViewModel 的
  “选择→下载→等待确认”；Compose UI 测试源码编译与 `:app:assembleDebug` 均通过。真实 WebDAV/S3 的
  远端枚举、下载、取消、断网恢复及完整恢复仍待用户实机验收，验收前不把
  本项标记为端到端完成。
- [x] **S3 第一阶段代码与自动测试（2026-09-06）。** 已实现端点/密钥/存储桶/区域/路径样式/远程目录设置、
  Keystore 加密、HEAD 存储桶连接测试、SigV4 完整载荷签名、流式上传、条件写入、区域重签和脱敏错误分类；
  备份模块 32 项 JVM 测试全绿，其中覆盖 AWS 官方签名示例、请求路径/载荷/进度、`auto` 区域重试和输入校验，
  Compose UI 测试源码亦已编译通过，`:app:lintDebug` 与完整 Debug 构建通过。Debug APK 已安装到 Motorola
  XT2551-3（Android 16），实机确认可从“备份与恢复”进入独立“网络存储设置”页并切换 WebDAV/S3，S3 端点、
  Access Key、Secret Key 与存储桶字段均正常显示。单 worker minified Release 也已构建通过，本地工作树 APK
  为 7,377,962 bytes（SHA-256 `c234b714aa27b9b3f94afceb861efe454e303e76fad6d2e61f87f0b7316633c8`，尚未发布）。
  真实 AWS S3、R2、MinIO 上传与各类权限/断网/取消场景仍需实机专项验收后才能标记端到端完成。

四项的建议顺序为：先完成设置内多账户管理，再实现内置编辑器；网络存储可复用备份流式传输后接入；
本机青龙服务端先做外部伴随 PoC，只有在沙箱生命周期、依赖和后台限制均有可接受证据后，才评估 App
私有目录内嵌方案。

### 长期路线（不纳入当前发布完成定义）

- English 全页面实机/截图矩阵和更广泛的可访问性回归；客户端文案、业务错误、瞬时提示、
  日期、数字和复数的代码迁移已于 2026-09-10 完成，不再列入长期实现项。
...
- 本机 Linux/青龙服务端探索已暂时放弃；网络存储的兼容矩阵、自签名证书与远端完整性复核等增强继续作为
  长期项，详见上方“2026-09-04 新增需求”章节，不改变当前版本发布门槛。
...
- [x] 切换后当前界面即时生效（允许安全重建 Activity），重启、退出登录和切换账户后
  仍保持选择；“跟随系统”随系统语言变化。
- [x] 客户端拥有的导航、对话框、Toast、Snackbar、空状态、校验错误和无障碍描述均接入中英文入口；
  2026-09-10 补齐账户页、通知、网络备份、依赖、脚本和仓储层业务错误。服务端返回的日志、源码和正文仍
  保持原样，只翻译客户端上下文。
- [x] 2026-09-10 任务时间和网络备份时间改为按当前 App Locale 使用系统短/中日期格式；动态任务、变量、
  标签、地址及瞬时消息补齐英文单复数，首页统计数字按当前 Locale 分组；新增共享本地化单元测试。
- [x] 2026-09-10 `:core:ui:testDebugUnitTest`、受影响的 settings/task/env/script/mcp 模块测试、
  `:app:testDebugUnitTest` 与 `:app:assembleDebug` 均通过。
- [ ] 中英文下分别完成登录、首页、脚本、环境变量、备份、依赖和设置页面全量实机/截图冒烟；代码门禁
  已完成，视觉、截断和 TalkBack 验收仍单独保留。
...
- [x] 生物识别应用锁使用系统 `BiometricPrompt` 与设备凭据回退，锁定时启用安全窗口，
  取消验证后只显示锁屏遮罩。
- [x] 2026-09-10 已完成客户端拥有文案、ViewModel 业务错误、Toast/Snackbar 等瞬时提示、
  日期、数字和英文单复数迁移；服务端返回的日志、脚本源码和正文保持原样。全页面截图、
  大字体与 TalkBack 实机矩阵仍作为独立验收项保留，不再误写成代码迁移未完成。
...
- [x] 接入官方两步验证初始化、启用和停用 API，保留密钥复制、
  验证码确认和停用确认。
- [x] 实现通知设置：已确认 `GET/PUT /api/user/notification`；动态字段、版本兼容、脱敏规则、测试通知语义
  与验收清单已统一收敛到“2026-09-09：通知设置与按日系统日志实施”，后续以该节为唯一实施入口。
...
- 青龙提供 `GET /api/subscriptions/{id}/log`，支持 `offset`、`limit` 和 `tail`；部分版本
  返回 `offset`、`nextOffset`、`total`、`truncated`，当前实测的青龙 2.21.0 仅保证正文可用、游标字段为零值。
  客户端同时支持标准游标增量与无游标全量快照兼容模式。
...
- [x] 日志内容不写入普通应用日志，不复制 Token 或私有仓库凭证；复制日志必须由用户
  主动触发。
- [x] Repository/ViewModel 测试覆盖日志元数据、正常增量、重叠全量响应、无游标快照、快速终态和陈旧状态启动探测；
  XT2551-3 真机已验证运行中日志增长、终态停止与最终正文不重复。空日志、分页、接口失败和切换时取消沿用既有测试，
  网络恢复属于后续韧性扩展，不再阻断本项完成定义。
...
### F. P2：服务端通知同步到 Android 本地通知（客户端接收基础已实现，中继待部署）
...
- [x] 2026-09-10 已加入由环境变量控制、默认关闭的 Firebase 初始化和
  `FirebaseMessagingService`；接收端只接受有版本、长度与标识符约束的 data payload，按服务器创建通知
  Channel，锁屏隐藏正文，不接受远端 URI/Intent/图标。缺少 Firebase 参数时不初始化、不影响应用核心功能。
- [x] 已新增 `docs/NOTIFICATION_RELAY_SECURITY.md`，确定配对、每账户接收密钥、FCM Token、限流去重、
  零正文持久化、撤销和青龙配置保护边界。
- [ ] 尚未完成中继服务部署、设备配对/Token 注册 UI、青龙 Webhook 安装/回滚与端到端实机推送；这些步骤
  需要实际 Firebase 项目参数、可达的中继地址和部署端凭据，不能把占位值或服务账户私钥写入仓库。
