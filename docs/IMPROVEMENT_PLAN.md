...
- [ ] **P2：MCP 服务增加“局域网可访问”与全写入授权档位。** 设置页提供“仅本机访问”（默认）与
  “局域网可访问”（显式开启）选择，并为单个 Agent 增加“预授权全部当前已注册写入/执行工具”的高风险档位。
  “局域网可访问”的底层监听会覆盖设备活动网络接口，也可能包含热点或 VPN；当前明文 HTTP Bearer Token 不适合直接对不可信网络开放。
  实现前必须确定 TLS/安全隧道或等效传输保护、可访问网段/接口限制、设备身份验证、强警告、实际监听地址展示、
