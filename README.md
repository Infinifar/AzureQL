...
- 📝 **大脚本安全工作流** — 脚本以文件流写入应用私有临时目录，按段预览、本地编辑、冲突确认后回传，并自动清理 8 天前草稿
- 🧭 **类型安全导航** (`@Serializable` routes)
- 📊 **首页仪表盘** — 任务总览卡 + 系统状态卡（内存 / CPU / 运行时长）
- 🗂️ **功能模块** — 定时任务、环境变量、脚本、订阅、依赖与日志管理
- ⏱️ **青龙 2.21 任务编辑** — 常规/手动/开机运行、附加定时、标签、实例模式、日志目录与执行前后命令
...
- 脚本下载使用青龙官方文件流接口，避免把大文件包装成一个巨大 JSON 字符串。小于等于
  512 KiB 的 UTF-8 文件可在应用内编辑；512 KiB～10 MiB 文件使用 32K 字符分段预览，
  并可交给系统文本编辑器修改；超过 10 MiB 的文件仅预览和下载。
- 编辑后的文件通过 multipart 文件流上传，回传前会比较服务端 `mtime`、大小或原始文件
  哈希；发现脚本已被其他客户端修改时必须由用户确认是否覆盖。非法 UTF-8 文件不会被
  替换字符后误写回服务端。
